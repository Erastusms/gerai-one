...
export class CartController {
  async handleAddToCart(
    ...
    return reply
      .status(201)
      .send(createSuccessResponse('Item added to cart successfully', result));
  }

  async handleGetCart(request: FastifyRequest, reply: FastifyReply) {
    const userId = request.user!.id;
    const result = await cartService.getCart(userId);
    return reply
      .status(200)
      .send(createSuccessResponse('Cart retrieved successfully', result));
  }

  async handleUpdateQuantity(
    request: FastifyRequest<{
      Params: { id: string };
      Body: UpdateQuantityInput;
    }>,
    reply: FastifyReply,
  ) {
    const userId = request.user!.id;
  ...
  async handleRemoveAllItems(request: FastifyRequest, reply: FastifyReply) {
    const userId = request.user!.id;
  ...
  async handleSelectItem(
    ...
    const userId = request.user!.id;
    const { id } = request.params;
    const result = await cartService.selectItem(
      userId,
      id,
  ...
  async handleRemoveSelected(request: FastifyRequest, reply: FastifyReply) {
    const userId = request.user!.id;
