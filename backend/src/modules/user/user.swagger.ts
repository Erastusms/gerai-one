import { updateProfileSchema, userResponseSchema } from "./user.schema";
...
export const getProfileSwagger = {
  schema: {
    ...
    security: [{ BearerAuth: [] }],
    response: {
      200: apiSuccessResponseSchema(userResponseSchema).describe("User profile retrieved"),
...
export const updateProfileSwagger = {
  schema: {
    ...
    response: {
      200: apiSuccessResponseSchema(userResponseSchema).describe("User profile updated successfully"),
...
export const deleteProfileSwagger = {
  schema: {
    ...
    security: [{ BearerAuth: [] }],
    response: {
      200: apiSuccessResponseSchema(userResponseSchema).describe("Profile soft deleted successfully"),
      401: apiErrorResponseSchema.describe("Invalid session token"),
    },
  },
};
